...
func TestClientContactsReturnsVisibleAppsUsersAndGroups(t *testing.T) {
	...
	if joinedPrivate["member_count"] != float64(2) {
		t.Fatalf("joined private group member_count = %v, want 2", joinedPrivate["member_count"])
	}
	joinedMembers, ok := joinedPrivate["avatar_members"].([]any)
	if !ok || len(joinedMembers) != 2 {
		t.Fatalf("joined private group avatar members = %#v, want two", joinedPrivate["avatar_members"])
	}
	for _, rawMember := range joinedMembers {
		member := rawMember.(map[string]any)
		if _, exists := member["email"]; exists {
			t.Fatalf("avatar member exposes email: %#v", member)
		}
		if _, exists := member["phone"]; exists {
			t.Fatalf("avatar member exposes phone: %#v", member)
		}
	}
	...
	if _, ok := groupsByID[publicGroup.ID]["avatar_members"]; ok {
		t.Fatalf("unjoined public group avatar members = %#v, want omitted", groupsByID[publicGroup.ID]["avatar_members"])
	}
