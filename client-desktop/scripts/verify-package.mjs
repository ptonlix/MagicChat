...
else await stat(path.join(applicationRoot, platform === "win" ? "MagicChat.exe" : "magicchat-desktop"))

console.log(JSON.stringify({ appId: "com.magicchat.desktop", arch, platform, version: packageJson.version }))
...
async function verifyMac(applicationRoot) {
  ...
  assert(arbitraryLoads === "false" && localNetworking === "false", "macOS ATS 未关闭不安全网络例外")
