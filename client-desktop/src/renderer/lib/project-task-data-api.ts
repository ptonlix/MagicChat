...
export async function deleteClientProjectTask(
  ...
  const payload = await readJson<
    ProjectTaskDataErrorEnvelope | ProjectTaskDataSuccessEnvelope<DeleteProjectTaskResponse>
  >(response)
  if (!response.ok || payload?.success === false) {
    throw createProjectTaskRequestError(payload, response, "删除任务失败")
  }
  const deletedTaskId = (
    payload as ProjectTaskDataSuccessEnvelope<DeleteProjectTaskResponse> | undefined
  )?.data?.task_id
