...
describe("message forwarding API", () => {
  it("sends one unified request and normalizes merged messages", async () => {
    ...
    const bundle = result.results[0].messages[0].body as ClientForwardBundleMessageBody
    expect(bundle.items[1].body.type).toBe("forward_bundle")
    expect((bundle.items[1].body as ClientForwardBundleMessageBody).items[0].summary).toBe("[图片]")
  })
})
