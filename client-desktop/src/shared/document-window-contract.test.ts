...
describe("文档窗口 Shared 契约", () => {
  it("只接受 documentId 和 serverId，并规范化文档 UUID", () => {
    expect(
      parseDocumentWindowRequest({ documentId: documentId.toUpperCase(), serverId: "server-1" }),
    ).toEqual({ documentId, serverId: "server-1" })
    expect(() =>
      parseDocumentWindowRequest({ documentId, serverId: "server-1", url: "/" }),
    ).toThrow("字段无效")
    expect(() =>
      parseDocumentWindowRequest({ documentId: "not-a-uuid", serverId: "server-1" }),
    ).toThrow("文档标识无效")
    expect(() => parseDocumentWindowRequest({ documentId, serverId: "server/1" })).toThrow(
      "服务器标识无效",
    )
  })

  it("构造显式本地路由，并按完整目标隔离窗口键", () => {
    const request = { documentId, serverId: "server-1" } as const
