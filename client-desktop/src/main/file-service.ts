...
export class FileService {
  ...
  private readonly activeUploads = new Map<
    string,
    { controller: AbortController; serverId: string }
  >()
  ...
  async upload(
    ...
    const body = Readable.from(
      (async function* () {
        yield prefix
        yield* createReadStream(picked.path)
        yield suffix
      })(),
    )
    ...
    try {
      const response = await this.sessions
        .for(profile)
        .fetch(`${profile.normalizedUrl}${assertClientPath(apiPath)}`, {
  ...
  async cleanup(): Promise<void> {
    ...
    await Promise.all(
      [...this.temporary.keys()].map((filePath) =>
  ...
  hasActiveTransfers(): boolean {
    return this.activeUploads.size > 0 || this.temporary.size > 0
  }
}
