...
export class CredentialStore {
  ...
  async set(serverId: string, accountId: string, secret: string): Promise<void> {
    ...
    await writeFile(this.entry(serverId, accountId), safeStorage.encryptString(secret), {
      mode: 0o600,
    })
  }
  ...
  async get(serverId: string, accountId: string): Promise<string | undefined> {
    ...
    try {
      return safeStorage.decryptString(await readFile(this.entry(serverId, accountId)))
    } catch {
      return undefined
    }
  }
  ...
  async removeServer(serverId: string): Promise<void> {
    ...
    for (const name of await readdir(this.directory).catch(() => []))
      if (name.startsWith(prefix)) await rm(path.join(this.directory, name), { force: true })
  }
