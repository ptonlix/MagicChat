...
export function parseDeepLink(rawUrl: string, knownServers: ReadonlySet<string>): DeepLinkAction {
  ...
  for (const key of url.searchParams.keys())
    if (/token|cookie|session|password|secret/i.test(key)) throw new Error("深链接包含敏感参数")
  const parts = url.pathname.split("/").filter(Boolean)
  if (
    url.hostname !== "v1" ||
    parts[0] !== "server" ||
    parts[2] !== "conversation" ||
    parts.length !== 4
  )
    throw new Error("深链接路由无效")
  const [serverId, conversationId] = [parts[1], parts[3]]
  if (![serverId, conversationId].every((value) => /^[a-zA-Z0-9-]{1,128}$/.test(value)))
    throw new Error("深链接标识无效")
