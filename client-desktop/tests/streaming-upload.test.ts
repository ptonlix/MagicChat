...
describe("流式上传", () => {
  ...
  it("拒绝超过 256 KiB 的单个 IPC 分块", async () => {
    ...
    await expect(controller.chunk(8, streamId, new Uint8Array(256 * 1024 + 1))).rejects.toThrow(
      "分块",
    )
  ...
  it("移除服务器时只中止该服务器的活动上传", async () => {
    ...
    controller.start(9, target, {
      headers: { "content-type": "multipart/form-data; boundary=first" },
      method: "POST",
      path: "/api/client/temporary-files",
      requestId: "request_3",
    })
    const otherStream = controller.start(
      9,
      { ...otherProfile, userId: "user-2" },
      {
        headers: { "content-type": "multipart/form-data; boundary=second" },
        method: "POST",
        path: "/api/client/temporary-files",
        requestId: "request_4",
      },
