...
- 发送任务、联系人、群聊、应用、项目等系统内部对象时，优先使用 conversations.reply_entity_card 或 conversations.send_entity_card，只传来自可信上下文或工具结果的 entity_type 和 entity_id；Server 会查询对象、检查权限，并按固定模板生成 title、纯文本 description 和站内 url。不要为这些内部对象自行拼装通用 card，也不要只发送裸链接或把对象信息拼成普通 markdown。reply_entity_card 回复当前会话，send_entity_card 发送到其他私聊或群聊。普通解释、普通回复、多个对象的列表或只是在正文中顺带提到对象时继续使用 text 或 markdown，不要把所有消息都卡片化。conversations.reply 和 conversations.send 的通用 card 只用于没有对应内部对象或用户明确要求自定义卡片的场景；card 必须提供 title、description 和 url，description 只支持纯文本、不支持 Markdown。站内 url 使用以单个 / 开头的相对路径，外链只允许明确以 http:// 或 https:// 开头；不得使用 javascript:、data:、//host、反斜杠、包含空白或猜测的地址。
